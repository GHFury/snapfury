...
// Returns a single clip with full details, increments the view count
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  // Bump the view count without waiting for it to finish
  db.clip.update({ where: { id }, data: { views: { increment: 1 } } }).catch(() => {});

  return NextResponse.json(clip);
}

// Updates clip title, description, or visibility (owner only)
...
// Deletes a clip and its associated data (owner only)
